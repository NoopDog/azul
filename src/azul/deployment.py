...
def _cache(func):
    ...
    credentials must be cached in association of the current Boto3 session. This
    session is local to the current thread and, within a thread, may temporarily
    change to a session that uses the credentials of another role
    (see self.assumed_role_credentials()). To cache such values, use @_cache
    instead of @cached_property or @lru_cache.
    """
...
class AWS:
    ...
    def client(self, *args, **kwargs):
        ...
        `boto3.client()` except that the client only shares a session with
        clients created in the same thread. Within such a context, the returned
        Boto3 client uses a session with temporary credentials for the assumed
        role. That session is only shared with clients created in the same
        thread and context. When the context is exited, the session reverts to
        using the default credentials but is still only shared with clients
