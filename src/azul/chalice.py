...
class ThrottledRoute:
    ...
    def sanitized_path(self):
        return re.sub(r'[/}{]', '_', self.path)

    @property
    def method_path(self):
        # https://github.com/hashicorp/terraform-provider-aws/issues/9738
        return f"{self.path.replace('/', '~1')}/{self.method}"

    @property
    def name(self):
        return f'{self.sanitized_path}___{self.method.lower()}'
...
class AzulChaliceApp(Chalice):
    ...
    def elasticsearch_throttling(self):
        ...
        return ThrottlingLimits(rate=10, burst=5)
