...
# TODO: Perform validation when API specification is added
...
catalog_param_spec = params.query(
    ...
    schema.optional(schema.with_default(app.catalog,
                                        type_={
                                            **schema.pattern(IndexName.catalog_name_re),
...
def repository_id_spec(index_name_singular: str):
    ...
    return {
        ...
        'parameters': [
            catalog_param_spec,
            params.path(f'{index_name_singular}_id', schema.uuid(), description=f'The UUID of the desired'
                                                                                f' {index_name_singular}')
        ],
        'responses': {
            '200': {
                'description': format_description(f'''
...
@app.route('/fetch/dss/files/{file_uuid}',
           methods=['GET'],
           validate=False,
           cors=True)
...
@app.route(drs.drs_object_url_path('{file_uuid}'),
           methods=['GET'],
           cors=True,
           validate=False,
           method_spec={
...
@app.route(drs.drs_object_url_path('{file_uuid}', access_id='{access_id}'),
           methods=['GET'],
           cors=True,
           validate=False,
           method_spec={
...
@app.route(drs.dos_object_url_path('{file_uuid}'),
           methods=['GET'],
           cors=True,
           validate=False)
